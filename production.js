...
memberLoginBtn.onclick=()=>{openModal('<div class="modal-top"><div><div class="eyebrow">Member Login</div><h3>Welcome back.</h3></div><button class="close" onclick="closeModal()">×</button></div><form id="zMember" style="margin-top:18px"><label>Phone number<input id="zMP" required inputmode="tel" placeholder="+91..."></label><label>Password<input id="zMpw" type="password" required placeholder="lg@26"></label><button class="btn" type="submit">Enter Course</button><div class="legal">Use your phone number with the member password to access the course.</div></form>');zMember.onsubmit=async e=>{e.preventDefault();try{const d=await api("login",{method:"POST",body:JSON.stringify({phone:zMP.value,password:zMpw.value})});if(d.user.role!=="student")throw Error("Use member login for the course.");save(d);closeModal();member()}catch(e){fail(e)}}};
